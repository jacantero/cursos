...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.transpose(tf.reshape(a_S, shape=[_, -1, n_C]))
    a_G = tf.transpose(tf.reshape(a_G, shape=[_, -1, n_C]))
    ...
    J_style_layer = tf.reduce_sum(tf.square(tf.subtract(GS,GG)))/(4*tf.square(float(n_H*n_W))*tf.square(float(n_C)))
