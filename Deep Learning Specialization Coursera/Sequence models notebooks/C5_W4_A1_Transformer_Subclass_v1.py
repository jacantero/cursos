...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        self_mha_output,_ = self.mha(x, x, x, return_attention_scores=True, training=training, attention_mask=mask)  # Self attention (batch_size, input_seq_len, fully_connected_dim)
        
        # skip connection
        # apply layer normalization on sum of the input and the attention output to get the  
        # output of the multi-head attention layer (~1 line)
        skip_x_attention = self.layernorm1(self_mha_output + x)  # (batch_size, input_seq_len, fully_connected_dim)
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, enc_output, training, look_ahead_mask, padding_mask):
        ...
        mult_attn_out1, attn_weights_block1 = self.mha1(x, x, x, return_attention_scores=True, training=training, attention_mask=look_ahead_mask)  # (batch_size, target_seq_len, d_model)
        ...
        mult_attn_out2, attn_weights_block2 = self.mha2(Q1, enc_output, enc_output, training=training, return_attention_scores=True, attention_mask = padding_mask)  # (batch_size, target_seq_len, d_model)
